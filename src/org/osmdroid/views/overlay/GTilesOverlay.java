 ...
 * 
 */
public class GTilesOverlay extends Overlay {

	private static final boolean DEBUGMODE = false;
	private static final Logger logger = LoggerFactory.getLogger(GTilesOverlay.class);
	...
	public GTilesOverlay(final MapTileProviderBase aTileProvider, final Context aContext) {
		this(aTileProvider, new DefaultResourceProxyImpl(aContext));

	}

	public GTilesOverlay(final MapTileProviderBase aTileProvider, final ResourceProxy pResourceProxy) {
	...
	public void draw(final Canvas c, final MapView osmv, final boolean shadow) {
		 ...
		 * one.
		 */
		Point centerXY = Mercator.projectGeoPoint(osmv.getMapCenter().getLatitudeE6() / 1E6, osmv.getMapCenter()
				.getLongitudeE6() / 1E6, zoomLevel, new Point(0, 0));
		final int tileNeededToLeftOfCenter = centerXY.x - 3;
		final int tileNeededToRightOfCenter = centerXY.x + 3;
		final int tileNeededToTopOfCenter = centerXY.y - 3;
		final int tileNeededToBottomOfCenter = centerXY.y + 3;
